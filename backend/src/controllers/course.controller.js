...
export async function getCourse(req, res) {
  try {
    ...
    //Validación de las variables del query
    const { error } = courseQueryValidation.validate({ id, nombre, idBossTeacher, idClassroom});

    if (error) return handleErrorClient(res, 400, error.message);

    const [course, errorCourse] = await getCourseService({ id, nombre, idBossTeacher, idClassroom});
...
export async function updateCourse(req, res) {
  try {
    ...
    if (courseError) return handleErrorClient(res, 400, "Error modificando al curso", courseError);
...
export async function deleteCourse(req, res) {
  try {
    ...
    if (errorCourseDelete) return handleErrorClient(res, 404, "Error eliminado al curso", errorCourseDelete);
...
export async function createCourse(req, res) {
  try {
      ...
      if (errorNewCourse) 
        return handleErrorClient(res, 400, "Error de registro de curso", errorNewCourse);
